...
function importResolve(content, baseDir) {
	var regex = /@import[^'"]+?['"](.+?)['"];?/g;
...
module.exports	= function(opts) {
	return throught.obj(function(file, enc, cb) {
		...
		if (file.isStream()) {
			cb(new gutil.PluginError('gulp-import-resovle', 'Streaming not supported'));
			return;
		}
		...
		file.contents	= new Buffer(importResolve(file.contents.toString(), path.dirname(file.path)));
